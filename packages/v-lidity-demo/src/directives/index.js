...
export default {
  install(VueApp) {
    // console.log(VueApp)
    VueApp.directive("validity", {
      beforeMount: (el, binding) => {
        // console.log(binding)
        // console.log(Object.keys(binding.value)[0])
        const f = this.setupForm(el, binding.value)
        binding.instance[Object.keys(binding.modifiers)[0]] = f
        nextTick(() => {
  ...
  setupForm(form, modelObj) {
    ...
    for (let i = 0; i < form.elements.length; i++) {
      ...
      if (this.isInput(element)) {
        ...
        if (!hasModelValue) {
          console.warn(`[v-constrain]: Unable to match a model value for field ${element.name}`)
          continue
        }
